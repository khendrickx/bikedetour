...
- **Strava actually uses MapLibre GL aliased as `window.mapboxgl`**: The canvas DOM class is `maplibregl-canvas`, not `mapboxgl-canvas`. Strava assigns the MapLibre library to `window.mapboxgl`, so the interceptor works, but any DOM selector using `.mapboxgl-canvas` will find nothing. Use `map.getCanvas()` / `map.getCanvasContainer()` via the map API instead.
