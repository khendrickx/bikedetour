...
(function () {
  ...
  window.addEventListener('message', (e) => {
    ...
    if (type === 'RW_DATA' && activeMap) {
      applyData(activeMap, e.data.hindrances, e.data.brussels, e.data.diversions);
    }
  ...
  function applyData(map, hindrances, brussels, diversions) {
    ...
    if (hCount + bCount + dCount > 0) {
      console.log(`[RoadWorks] ${hCount} GIPOD hindrances, ${bCount} Brussels events, ${dCount} diversions`);
    }
  }
  ...
  function setVisible(map, visible) {
    ...
    [LAYER_FILL, LAYER_OUTLINE, LAYER_BRUSSELS_CIRCLE, LAYER_DIVERSION].forEach((id) => {
